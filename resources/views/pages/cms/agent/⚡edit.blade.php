...
new class extends Component
{
    ...
    {
        $this->agent = Mst_agent::findOrFail($id);

        // set default value from database
        $this->name = $this->agent->name;
        $this->agent_code = $this->agent->agent_code;
        $this->isActive = $this->agent->active=='Y'?true:false;
        ...
        // ambil kategori yg sudah dipilih
        $this->categoriesSelected = Mst_category::select(
            'id',
            'name'
        )
        ...
        ->get()
        ->toArray();
        foreach ($this->categoriesSelected as $key => $value) {
            if (!in_array($value['id'], $this->categories2)) {
                $this->categories2[] = $value['id'];
            }
        }
        ...
        // ambil achievement yg sudah dipilih
        $this->achievementsSelected = Mst_achievement::select(
            'id',
            'name'
        )
        ->whereIn('id', function($q) use($id){
        ...
        ->get()
        ->toArray();
        foreach ($this->achievementsSelected as $key => $value) {
            if (!in_array($value['id'], $this->achievements2)) {
                $this->achievements2[] = $value['id'];
            }
        }
    }
    ...
    protected function rules()
    {
        return [
            ...
            'agent_code' => ['required', 'max:10', new CheckAgentDupRule($this->agent->id)],
            'photo' => ['nullable', 'image', 'file', 'max:1024', 'dimensions:max_width=500,max_height=500'] ,
        ];
    }
    ...
    public function update()
    {
        ...
        if ($this->photo){
            ...
            if (file_exists($realpath.$this->agent->photo)) {
            // jalankan hapus file
                unlink($realpath.$this->agent->photo);
            }

            // simpan photo baru
            $this->photo->storeAs(path: 'agents', name: $this->agent->photo);

            $photoNm = $this->agent->photo;
        }else{
            // tidak ada photo yg di upload
            $photoNm = $this->agent->photo;
        }
        ...
        // update master category - agent - start
        $updSetNotActive = Mst_agent_category::where('agent_id', $this->agent->id)
        ->update([
            'active' => 'N',
            'updated_by' => 1,
        ]);
        ...
        foreach ($this->categories2 as $key => $value) {
            ...
            $qCheckCategory = Mst_agent_category::where('agent_id', $this->agent->id)
            ->where('category_id', $value)
            ->first();
            if ($qCheckCategory){
                $updSetActive = Mst_agent_category::where('agent_id', $this->agent->id)
                ->where('category_id', $value)
                ->update([
                    'active' => 'Y',
                    'updated_by' => 1,
        ...
        $updSetNotActive = Mst_agent_achievement::where('agent_id', $this->agent->id)
        ->update([
            'active' => 'N',
            'updated_by' => 1,
        ]);
        ...
        foreach ($this->achievements2 as $key => $value) {
            ...
            $qCheckAchievement = Mst_agent_achievement::where('agent_id', $this->agent->id)
            ->where('achievement_id', $value)
            ->first();
            if ($qCheckAchievement){
                $updSetActive = Mst_agent_achievement::where('agent_id', $this->agent->id)
                ->where('achievement_id', $value)
                ->update([
                    'active' => 'Y',
                    'updated_by' => 1,
