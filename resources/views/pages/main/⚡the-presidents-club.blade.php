...
new class extends Component
{
    ...
    public function render()
    {
        ...
        ->when($this->search=='', function($query){
            $query->whereIn('id', function($query1){
                ...
                ->when($this->q=='l', function($query1a) {
                    $query1a->where('category_id', 10);
                })
                ->when($this->q=='p', function($query1a) {
                    $query1a->where('category_id', 38);
                })
                ->when($this->q!='l' && $this->q!='p', function($query1a) {
                    $query1a->whereIn('category_id', [10, 38]);
                })
                ->where('active', 'Y');
            });
        })
        ...
        //         $query1->where('category_id', 10);
        //     })
        //     ->when($this->q=='p', function($query1) {
        //         $query1->where('category_id', 38);
        //     })
        //     ->when($this->q!='l' && $this->q!='p', function($query1) {
        //         $query1->whereIn('category_id', [10, 38]);
        //     })
        //     ->where('active', 'Y');
        // })
        ->when($this->search!='', function($query){
    ...
    <div class="team-section">
        <div class="container transparan">
            ...
            <div align="center" class="mb-30">
                <img src="assets/imgs/b_tpc.png" class="tulisan_tac" alt=""/> 
            </div>
            ...
            {{-- <div class="d-flex justify-content-center align-items-center mb-30">
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                        <li><a class="dropdown-item" href="{{ route('the-presidents-club').'?q=l' }}">Leader </a></li>
                        <li><a class="dropdown-item" href="{{ route('the-presidents-club').'?q=p' }}">Producer </a></li>
                    </ul>
                </div> 
            </div> --}}
            ...
            <div class="d-flex justify-content-center align-items-center mb-30">
                <div class="flat-select-container">
                    <select id="top-regional-area" class="flat-select" name="kategori_pilihan" onchange="goSubCategory('{{ route('the-presidents-club').'?q=' }}'+this.value);">
